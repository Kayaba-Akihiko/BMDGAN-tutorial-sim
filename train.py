...
from dataset import Stage, TrainingDataset, TestDataset, VisualizationDataset
...
class Trainer:
    def __init__(
        ...
        self._tb_writer.add_graph(self._netG, torch.randn(1, 3, *image_size, device=self._device))
    ...
    def visual_epoch(self, epoch):
        ...
        for data in self._data_module.visualization_data_loader:
            ...
            pet_image = data["pet_image"].to(self._device)
            pred_pet_image = self._netG(image)
            norm_mean = data["norm_mean"][..., None, None].to(self._device)  # (B, C, 1, 1)
            norm_std = data["norm_std"][..., None, None].to(self._device)  # (B, C, 1, 1)

            tag_image_dict = {"image": image, "pet_image": pet_image, "pred_pet_image": pred_pet_image}

            for tag, img in tag_image_dict.items():
                # denormalize -> (0, 1) -> (0, 255)
                img = (img * norm_std + norm_mean).clamp(0., 1.)
    ...
    def eval_epoch(self, epoch):
        ...
        for data in tqdm(iterator, desc="Testing", mininterval=30, total=n_iter, maxinterval=60):
            ...
            # denormalize -> (0, 1)
            pet_image = (pet_image * norm_std + norm_mean).clamp(0., 1.)
            pred_pet_image = (pred_pet_image * norm_std + norm_mean).clamp(0., 1.)

            psnr += calc_psnr(pred_pet_image, pet_image).sum()
            ssim += calc_ssim(pred_pet_image, pet_image).sum()
            total_count += pet_image.shape[0]
    ...
    def _compute_loss(self, data: dict[str, Any], epoch_loss_logger: EpochLossLogger):
        ...
        l1_loss = self._criterion_l1(fake_drr, drr)
        epoch_loss_logger.log("G_L1", l1_loss)
        g_loss_total = g_loss_total + l1_loss * 100.

        gc_loss = self._criterion_gc(fake_drr, drr)
        epoch_loss_logger.log("G_GC", gc_loss)
        g_loss_total = g_loss_total + gc_loss * 1.

        d_pred_fake = self._netD(fake_drr)
        d_pred_real = self._netD(drr)

        fm_loss = self._criterion_fm(pred_fake=d_pred_fake, pred_real=d_pred_real)
        epoch_loss_logger.log("G_FM", fm_loss)
        g_loss_total = g_loss_total + fm_loss * 10.

        gan_loss = self._criterion_gan(d_pred_fake, target_is_real=True)
        epoch_loss_logger.log("G_GAN", gan_loss)
        g_loss_total = g_loss_total + gan_loss * 1.
